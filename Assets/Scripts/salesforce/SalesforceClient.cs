...
namespace Salesforce
{
    public class SalesforceClient : MonoBehaviour {
        ...
        */
        ...
        /*
        * @description Executes a get Content Version Call
        *
        * @param cvid the ContentVersion ID to be queried
        * @return base64 encoded string of returned file
        * @throws SalesforceApiException if query fails
        */
        public IEnumerator retrieveContentVersionData(string cvid) {
            assertUserIsLoggedIn();
            string url = getDataServiceUrl() +"sobjects/ContentVersion/"+ UnityWebRequest.EscapeURL(cvid) + "/VersionData";
            using (UnityWebRequest request = getBaseRequest(url, "GET")) {
                yield return request.SendWebRequest();
                if (request.isNetworkError || request.isHttpError) {
                    logResponseError(request);
